...
export default function VideoPage({ post, videoId, profileId }: TvideoPage) {
  ...
  const {
    ...
    error: statusError,
  } = useNoteStatus(profileId, videoId);
...
*/
  ...
  const {
    data: videoComments,
    isLoading: isVideoCommentsLoading,
    isError: isVideoCommentsError,
    error: videoCommentsError,
 ...
 */
  ...
  } = useQuery(["profile-data"], fetchProfileData);
 ...
 */
  const handleTip = async (tip, i) => {
    ...
    setPointedAmount(i);
    try {
      const result = contract.tipCharacter(
        character?.characterId,
        profileData?.data.characterId,
        amount.toString()
      );
    } catch (error) {
...
*/
  ...
  const handleLikeVideo = async () => {
    try {
      const { data } = await contract.linkNote(
        character?.characterId,
        profileId,
        videoId,
        "like"
 ...
 */
  ...
  const handleSubscribe = async () => {
    try {
      const { data } = await contract.linkCharacter(
        character?.characterId,
        profileId,
        "follow"
      );
                ...
                <Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-2xl bg-white dark:bg-gray-900 p-4 text-left align-middle shadow-xl  transition-all">
                  <Dialog.Title className="text-lg font-medium leading-6 ">
                    <div className="flex justify-between">
                      ...
                      <AiOutlineClose
                        onClick={toggleShowTipModal}
                        className="opacity-75 cursor-pointer"
                      />
                    </div>
                    <div className="flex items-center justify-center flex-col ">
                      <CharacterAvatar
                        character={profileData?.data}
                        size={60}
                        className="xl:w-[200px]"
                      />
                      {profileData?.data.metadata.content.bio && (
                        <p className="text-sm">
                          {shortenTxt(
                            profileData?.data.metadata.content.bio,
                            20
                  ...
                  <div className="mt-4">
                      ...
                      {tipsTires.map((tip, i) => {
                        return (
                          <div
                            key={i}
                            className={`${
                              pointedAmount === i && "border-blue-600"
                            } flex gap-2 border border-gray-300 dark:border-gray-600 px-3 py-2 rounded-lg cursor-pointer`}
                    ...
                    <div className="flex gap-2 items-center justify-center my-3">
                      ...
                      <a
                        ...
                        className="outline-none font-medium text-gray-500"
                      >
                        What is MIRA? where I can get some?
                      </a>
                    </div>
                  </div>
                </Dialog.Panel>
        ...
        <div className="xl:w-[73vw] px-3 ">
          ...
          <div className="flex justify-between xs:flex-col md:flex-row">
            <div className="flex gap-4">
              ...
              <p className=" font-semibold opacity-70">
                {status?.mintCount} Mints
              </p>
              <p className=" font-semibold opacity-70">
                Uploaded {duration.humanize().replace("a ", "")} ago
              </p>
            </div>
            <div className="flex gap-3 items-center ">
              <div
                className={`flex gap-2 border hover:bg-gray-200 py-0.5 px-1 items-center rounded-md cursor-pointer ${
              ...
              <div
                className={`flex gap-2 hover:bg-gray-200 py-0.5 px-1 items-center rounded-md cursor-pointer ${
                  status?.isMinted || isVideoMinted
                    ? "text-blue-600"
                    : "text-gray-900 dark:text-white"
                }`}
                onClick={handleMint}
              ...
              <div>
                <AiOutlineDollar
                  className="xs:w-5 xs:h-5 md:w-6 md:h-6 cursor-pointer"
                  onClick={toggleShowTipModal}
                />
              </div>
              <TbShare3
          ...
          <div className="flex justify-between items-center mt-3">
            <div className="flex gap-3">
              ...
              <div>
                <Link
                  href={`/channel/${profileData?.data.characterId}`}
                  className="opacity-90"
                >
                  {profileData?.data.handle}
                </Link>
                <p className="text-xs opacity-80">
                  Subscribers {profileStats?.followersCount}
                </p>
              </div>
            </div>
            <button
            ...
            >
              {relationStatus?.isFollowed ? "Subscribed" : "Subscribe"}
            </button>
          </div>
          {/*DISPLAY MODALS */}
          {isTipModal && <TipUser />}
