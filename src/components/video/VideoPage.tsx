...
import { useAccountCharacter, useNoteLikeCount } from '@crossbell/connect-kit'
...
/*   
 =============================
 Get video total mints
 =========================
*/

 const getVideoMints = async () =>  {
  const result = await indexer.getMintedNotesOfNote(profileId, videoId, {
    limit : 0
  })
  settotalMints(result)
 }

 useEffect(() => {
   getVideoMints()
 }, [])
  console.log("vdeio mints", totalMints)
 /*   
 =============================
 Get video comments
 ===================
...
const COMMENTS_BASE_URL = `https://indexer.crossbell.io/v1/notes?limit=20&toCharacterId=${profileId}&toNoteId=${videoId}&includeDeleted=false&includeEmptyMetadata=false&includeCharacter=true&includeHeadCharacter=false&includeHeadNote=false&includeNestedNotes=true`
 ...
 const {data : videoCommentss, isLoading : isVideoCommentsLoading, isError : isVideoCommentsError, error: videoCommentsError} = useQuery(['comments-data'], fetchvideoComments)
            ...
            <div className='flex justify-between xs:flex-col md:flex-row'>
             <div className='flex gap-4'>
               <p className=' font-semibold opacity-70'>0 Views</p>
                 <p className=' font-semibold opacity-70'>0 Collects</p>
         <p className=' font-semibold opacity-70'>Uploaded {duration.humanize().replace("a ", "")} ago</p>
             </div>
              <div className='flex gap-3 items-center '>
                  <div className='flex gap-2 border hover:bg-gray-200 py-0.5 px-1 items-center rounded-md cursor-pointer'>
                     <p className='text-lg'>1</p>
                      <LikeOutline className='xs:w-4 xs:h-4 md:w-5 md:h-5' />
                      </div>
                  <div className='flex gap-2 hover:bg-gray-200 py-0.5 px-1 items-center rounded-md cursor-pointer '>
                    <p>1</p>
