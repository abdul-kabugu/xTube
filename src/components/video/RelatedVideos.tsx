...
export default function RelatedVideos() {
    ...
    const {data, isLoading, isFetching, error, isError} = useDiscover({limit: 20, locked: false, includeCharacter : true, includeDeleted : false, tags : []})
     if(isError) {
        return(
  ...
  return (
    <div className='xs:hidden xl:flex flex-col '>
     {data?.data?.list.map((item, i) =>  {
        ...
        return(
            <RelatedVideoCard key={i} video = {item} />
        )
     })}
    </div>
  )
