...
import { useConnectedAccount, ConnectButton, useXSettingsModal, useAccountCharacter, useIsConnected } from "@crossbell/connect-kit";
    ...
    <div className='mt-4'>
        ...
        <CommentOutline className='w-4 h-4' />
         ...
         <CharacterAvatar size={40} character={account?.character} />
            <input    className='py-2 px-3 rounded-md w-[80%] focus:outline-none bg-inherit border border-gray-300 dark:border-gray-700' placeholder='comment txt' value={commentTxt} onChange={e => setcommentTxt(e.target.value)}  />
             <button className='bg-blue-600 py-1 px-3 rounded-lg text-white' onClick={handleComment}>Comment</button>
         </div>
}
          <div>
            {comments?.list?.map((item, i) =>  {
                ...
                return(
                 <CommentCard comment={item} key={i}  />
                )
            })}
          </div>
