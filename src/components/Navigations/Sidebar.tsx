...
export default function Sidebar({ isExpanded, toggleSidebar }: Tsidebar) {
  return (
    <div
      className={`${
        isExpanded ? "w-[120px] " : "w-[70px] "
      }  border-r-1 h-screen fixed top-0  duration-500 ease-in-out xs:hidden md:block `}
    >
      <div>
          ...
          return (
            <Link href={item.to} key={i}>
              <div
                className={`${
                  isExpanded
                    ? "flex gap-3 items-center my-4 px-3 py-2 hover:bg-gray-200 dark:hover:bg-gray-700 rounded-lg"
                    : "flex flex-col my-5 px-3 items-center justify-center hover:bg-indigo-100 dark:hover:bg-gray-700  w-full h-12 py-1.5"
                }`}
              >
                <item.icon
